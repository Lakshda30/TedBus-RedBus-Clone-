...
handleproceedtopay():void{
  ...
  localStorage.setItem('paymentFlowData', JSON.stringify(routeParams));
  localStorage.setItem(this.paymentPassengerStorageKey, JSON.stringify(this.passdetails));
  localStorage.setItem(this.paymentRouteStorageKey, JSON.stringify(this.routedetails));
  this.dataservice.passobj(this.passdetails)
  this.dataservice.sendobj(this.routedetails)
  this.router.navigate(['/payment'], { state: routeParams })
}
}
