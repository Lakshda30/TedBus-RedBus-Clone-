...
The Google Sign-In button needs your own OAuth 2.0 Web client ID. In Google Cloud Console, create or open an OAuth client and add this Authorized JavaScript origin:

```text
http://localhost:4200
```

Then replace `YOUR_GOOGLE_CLIENT_ID.apps.googleusercontent.com` in `frontend/src/index.html` with that client ID. Use the same value for `GOOGLE_CLIENT_ID` in `frontend/server/.env` so the backend can verify the Google credential.
